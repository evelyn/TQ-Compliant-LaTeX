...
\newcommand{\indentForLevel}[2]{\dimexpr #2\baseSpaceLength * #1\relax} % Define a command that multiplies the length of ~ by two factors
...
% -------- # List settings
%%% itemindent=\spaceLength{4}, % no idea what this does
%%% itemsep=0em, % no idea what this does
%%% labelindent=0pt, % No idea what this does
%%% before=\setlength{\listparindent}{0pt}, % Continuation lines align with the left margin (Allegedly? Seems to already be doing that)


% First-level list (aligning with margin and wrapping properly)
\setlist[enumerate,1]{
    label=\arabic*., % Label text
    itemindent=\indentForLevel{1}{4}, % Controls the left of the margin and the left of the item
    ...
    labelsep=\spaceLength{2}, % \spaceLength{N} = ~ in label, can be kept as 0 if using ~~
                              % label + Distance between content
    leftmargin=\spaceLength{0}, % Aligns everything with the left margin
}

% Second-level list (indented first line, wraps to left margin)
\setlist[enumerate,2]{
    label=\alph*., % Label text
    itemindent=\indentForLevel{2}{4}, % Controls the left of the margin and the left of the item
                                      % The first argument adjusts the overall indentation level (e.g., nesting level)
                                      % The second argument sets the scaling factor for the indentation distance
    left=\spaceLength{0}, % Horizontal movement for everything
    labelsep=\spaceLength{2}, % \spaceLength{N} = ~ in label, can be kept as 0 if using ~~
...
\setlist[enumerate,3]{
    label=(\alph*), % Label text
    itemindent=\indentForLevel{3}{4}, % Controls the left of the margin and the left of the item
                                      % The first argument adjusts the overall indentation level (e.g., nesting level)
                                      % The second argument sets the scaling factor for the indentation distance
    left=\spaceLength{0}, % Horizontal movement for everything
    labelsep=\spaceLength{2}, % \spaceLength{N} = ~ in label; can be kept as 0 if using N=~ in label
                              % label + Distance between content
    leftmargin=\spaceLength{0}, % Aligns everything with the left margin
}

% Fourth-level list (further indented, same behavior)
\setlist[enumerate,4]{
    label=(\alph*), % Label text
    itemindent=\indentForLevel{4}{4}, % Controls the left of the margin and the left of the item
                                      % The first argument adjusts the overall indentation level (e.g., nesting level)
                                      % The second argument sets the scaling factor for the indentation distance
    left=\spaceLength{0}, % Horizontal movement for everything
    labelsep=\spaceLength{2}, % \spaceLength{N} = ~ in label; can be kept as 0 if using N=~ in label
...
% Fifth-level list (further indented, same behavior)
\setlist[enumerate,5]{
    label=(\alph*), % Label text
    itemindent=\indentForLevel{5}{4}, % Controls the left of the margin and the left of the item
                                      % The first argument adjusts the overall indentation level (e.g., nesting level)
                                      % The second argument sets the scaling factor for the indentation distance
    left=\spaceLength{0}, % Horizontal movement for everything
    labelsep=\spaceLength{2}, % \spaceLength{N} = ~ in label; can be kept as 0 if using N=~ in label
...
% Sixth-level list (further indented, same behavior)
\setlist[enumerate,6]{
    label=(\alph*), % Label text
    itemindent=\indentForLevel{6}{4}, % Controls the left of the margin and the left of the item
                                      % The first argument adjusts the overall indentation level (e.g., nesting level)
                                      % The second argument sets the scaling factor for the indentation distance
    left=\spaceLength{0}, % Horizontal movement for everything
    labelsep=\spaceLength{2}, % \spaceLength{N} = ~ in label; can be kept as 0 if using N=~ in label
...
\noindent
\begin{tabularx}{\textwidth}{@{} p{\referencesWidth} @{} >{\raggedright\arraybackslash}X @{}}
